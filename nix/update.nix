...
writeShellApplication {
  ...
  text = ''
    ...
    yq -cj '
      ...
      | tostring + "\u0000"
    ' "$toplevel"/config/west.yml | xargs -0 -L1 bash -c '
      set -euo pipefail
      ...
      head="$(grep -F "$currevision" "$toplevel"/config/west.yml | cut -d"#" -f2 | tr -d " ")"

      [ -z "$head" ] && exit 0

      newrevision="$(git ls-remote "$url" "$head" | sed -e "s/\t.*$//")"
      sed -i -e "s|$currevision|$newrevision|" "$toplevel"/config/west.yml
    ' "$toplevel"
